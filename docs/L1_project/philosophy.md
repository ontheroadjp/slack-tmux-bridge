...
- 1チャンネル⇔1ペインを維持しながら複数ペイン運用したい利用者。根拠: goslack.py:245-359

# 主要な設計原則
- 安全性: allowlist/denylist によるコマンド制御を行う。根拠: slack_tmux_bridge.py:150-168
- 継続性: tmux セッションを前提とし、セッション情報を active_sessions.json で保持する。根拠: goslack.py:267-283
- UX: 数字は自動実行、テキストはボタンで実行を確定する。根拠: slack_tmux_bridge.py:640-667
- 可観測性: イベント監視・プロンプトキャッシュの維持や通知を行う。根拠: slack_tmux_bridge.py:43-52,454-512
