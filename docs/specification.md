...
| `active_sessions.json` | Slack チャンネルID → tmux ターゲットのマッピング |
...
| `COMMAND_DENYLIST` | 空（→`rm`単独を拒否） | 拒否パターン | 破壊コマンド防止 |
| `OUTPUT_DIFF_MODE` | `replace` | 出力差分方式 | 誤削除の可能性に備えて切替可能 |
...
### 4.1 役割
- **現在の tmux ペイン**を取得し、`active_sessions.json` に**Slack チャンネルとの対応**を書き込む。
- 同一ペインを指す他チャンネルのエントリは**自動で削除**。

### 4.2 フロー
1. 現在の作業ディレクトリに対応するチャンネルIDを `config.json` から探す。
2. `tmux display-message` で `session:window.pane` を取得。
3. 既存の `active_sessions.json` から同一ペインの他エントリを削除。
4. 新しいチャンネル→ペインを登録（atomic 書き込み）。
...
- denylist 未指定時は `rm` 単独をブロック。

### 5.3 入力の種類
- **数字のみ**: 即実行（プリクリア → 入力 → Enter → 監視）。
- **テキスト**: tmux へ入力だけ送ってボタンを表示。
  - 「▶︎ 実行（Enter）」で Enter 送信と監視開始。
...
- ANSI エスケープを除去して比較に利用。

### 6.3 出力差分
- `OUTPUT_DIFF_MODE=replace`:
  - `current_clean.replace(initial_clean, "")` で差分抽出。
- `OUTPUT_DIFF_MODE=suffix`:
  - `current_clean` 内の**最後の initial_clean**位置以降を抽出。
- **なぜ**: `replace` は同じ文字列が複数回出現すると誤削除する場合があるため、切替可能にして運用で比較できるようにした。
...
### 7.2 抽出ロジック
1. 実行開始時の画面との差分を抽出。
2. `"> prompt"` 行を探し、その行以降のみ採用。
3. 先頭が `>` の場合は**プロンプト行の直後に空行**を挿入。
...
**なぜ**: 中途半端な回答を投稿すると意味がないため、完了を明確に検出する。

---

## 8. スナップショット管理

- 実行開始時の tmux 出力を `tmp/snapshot_<thread_ts>.txt` に保存。
- JSON 形式で `content` と `prompt` を保存。
- 完了時に削除、タイムアウト時は残す。
- `SNAPSHOT_TTL_SEC` を超えた古いファイルは自動削除。
...
1. `.env` を作成し、Slack トークンとチャンネル ID を設定。
2. tmux で Gemini を起動。
...
├── active_sessions.json  # チャンネル→ペイン対応表
├── tmp/                  # スナップショット/ PID
└── docs/
