...
- **テキスト**: 受信時に Slack 返信指示を付与して tmux へ入力を送り、ボタンを表示。
  - 「▶︎ 実行（Enter）」で Enter 送信のみを行う。
  - 「👀 Geminiを見る」現在の tmux 出力を送信。
  - 「🗑️ プロンプト削除」Ctrl+U で入力行を消去。
- **スラッシュコマンド**: `スラッシュコマンド` というメッセージによりボタンメニューを提示。

### 5.4 実行時の返信先指示
- **受信時**にプロンプト末尾へ返信指示文を付与して tmux に送る。
- 返信指示文は以下の形式（括弧のみ）:
  - `(<channel_id>チャンネルの<thread_ts>スレッドに実行結果を返信してください。許可を求める場合も同スレッドに投稿してください。)`
- 「▶︎ 実行（Enter）」は Enter 送信のみを行う。
- 実行結果の投稿は **AI エージェント自身**が行い、ブリッジは結果投稿を行わない。
